...
volatile bool nrfIrqTriggered = false;
...
uint8_t NRF_TxPacket( uint8_t *txbuf, uint8_t Length )
{
	...
	NRF_SET_CE_LOW( );		
	NRF_Write_Buf( nrfCMD_W_TX_PLD, txbuf, Length );	//写数据到TX BUF 32字节  TX_PLOAD_WIDTH
	NRF_SET_CE_HIGH( );			//启动发送\

	while( !nrfIrqTriggered )	
	{
		if( waitCount++ == 80000 )
			break;
	}
	...
	status = NRF_Read_Reg( nrfREG_STATUS );						//读状态寄存器
	NRF_Write_Reg( nrfREG_STATUS, status );						//清除TX_DS或MAX_RT中断标志
	
	if( status & nrfTX_OK )	//发送完成
	{
		return nrfTX_OK;
	}
	
	if( status & nrfMAX_TX )	//达到最大重发次数
	{
...
uint8_t NRF_SendData( uint8_t *pbSrc, uint16_t len )
{
	...
	while( len )
	{
		...
		size = ( len > FIXED_PACK_LEN ) ? FIXED_PACK_LEN : len;	
		
		res = NRF_TxPacket( p, size );
		if( res == nrfMAX_TX )
		{
			errCount++;
		}
...
void NRF_Init( void )
{
	...
	NRF_Write_Reg( nrfREG_SETUP_AW, nrfAW_5BYTES );				//地址宽度
	NRF_Write_Reg( nrfREG_SETUP_RETR, nrfARD_500US | 0x03 );	//重发延时和重发次数
	NRF_Write_Reg( nrfREG_RF_CH, RF_Channel );             		//初始化通道
	NRF_Write_Reg( nrfREG_RF_SETUP, 0x0e );

    NRF_SetTxAddr( (uint8_t *) TxAddr, 5 );                      //设置TX地址
    NRF_SetRxAddr( 0, (uint8_t *) TxAddr, 5 );                   //设置RX地址
}
...
int quickSend( uint8_t *pbuf, uint16_t size )
{
	...
	NRF_SET_CE_HIGH();
	NRF_Write_Tx_Payload_NoAck( pbuf + 32 * 0, 32 );
...
//	delay_us( 200 );
	...
	printf("status: %02X, duration: %d us\r\n", 
